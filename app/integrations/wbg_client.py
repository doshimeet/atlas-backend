...
def classify_thematic_practice(paper_dict: Dict[str, Any]) -> tuple:
    ...
    """
    text_corpus = " ".join([
        paper_dict.get("docna", {}).get("0", {}).get("docna", "") if isinstance(paper_dict.get("docna"), dict) else str(paper_dict.get("docna", "")),
...
async def fetch_live_wbg_publications(rows: int = 15, query: str = "") -> List[Dict[str, Any]]:
    ...
    """
    base_url = settings.WBG_API_BASE_URL
    params = {
        "format": "json",
        "rows": str(rows),
    ...
    headers = {
        "User-Agent": "AtlasKnowledge-ResearchEngine/3.2 (World Bank Open Knowledge Repository)",
